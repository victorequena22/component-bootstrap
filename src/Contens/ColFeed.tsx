...
export function ColFeed({ col, error, children }: Props & { children: any }) {
  return (
    <Col xs={col} className='mt-2' as={InputGroup} hasValidation={error ? true : false}>
      {children}
      <FormControl.Feedback type='invalid'>{error ? error : ''}</FormControl.Feedback>
    </Col>
  )
}
interface Selec {
