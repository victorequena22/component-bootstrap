...
export function ButtonColor({
    ...
    const handleChange = (c: Color) => { setData(`#${c.hex}`); setColor(c) };
    const handleClick = (event: any) => { setOpen(event.currentTarget); if (click) click() };
    ...
    return <>
        ...
        {disable ? <></> : <Popover
            ...
            <RowConfig style={{
                ...
                {colores.map(c => <div onClick={() => setData(c)} style={{ backgroundColor: c, border: '1px solid black', height: '25px' }} />)}
            </RowConfig>
        </Popover>}
    </>
}